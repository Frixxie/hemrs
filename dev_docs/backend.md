...
`GET /api/devices/{device_id}/sensors/{sensor_id}/measurements/stream` subscribes to the process-local broadcast channel and returns matching updates as named SSE `measurement` events. It loads the latest value from the cache or database when a client connects, emits it immediately, and rereads the latest value from PostgreSQL every 15 seconds. The refreshed value is also stored in the local cache. Keepalive comments are used when no value exists, and `X-Accel-Buffering: no` asks compatible proxies not to buffer the response.

The channel is bounded and deliberately lossy so slow subscribers cannot block ingestion. Lagged receivers resume with the next available update. Broadcast updates do not cross backend process boundaries, so another replica's insert can take up to 15 seconds to appear through the database refresh. There are no SSE event IDs or replay; shared pub/sub would still be required for immediate cross-replica delivery.
